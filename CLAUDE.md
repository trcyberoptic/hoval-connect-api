...
- **A release can be installed and verified end-to-end over the REST API alone, no SSH** (done 2026-08-09, ~50 s from restart to fresh state). With a long-lived token: `POST /api/services/homeassistant/update_entity` on `update.hoval_connect_update` to make HACS notice the tag, then `POST /api/services/update/install`, then `POST /api/services/homeassistant/restart`; poll the entity you changed until it leaves `unavailable`.
